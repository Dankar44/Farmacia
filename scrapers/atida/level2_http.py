"""
Level 2 HTTP scraper for Atida (atida.com)
...
SITEMAP_URL = "https://www.atida.com/es-es/sitemap.xml"
BATCH_SIZE = 100

# Config for extract_product_from_html
HTML_CONFIG = {
    'name_selector': 'h1',
    'price_selector': 'meta[property="product:price:amount"]',
    'price_attr': 'content',
    'stock_selector': None,  # We handle stock via custom logic below
    'ean_selector': '[itemprop="sku"], [itemprop="gtin13"]',
    'ean_attr': 'content',
}
...
def is_product_url(url):
    ...
    # Atida product URLs typically contain /es-es/ and a product slug
    if '/es-es/' not in url:
        return False
    # Skip category, info, and landing pages
    skip_patterns = [
        '/categoria/', '/categorias/', '/brands/', '/info/',
        '/login', '/registro', '/checkout', '/carrito',
        '/blog/', '/ayuda/', '/contacto',
    ]
...
def extract_ean_from_html(html_text):
    """Try to extract EAN from structured data or itemprop attributes."""
    ...
    if el:
        ...
        if val and re.match(r'^\d{7,13}$', val):
            return val
    ...
    for script in soup.find_all('script', type='application/ld+json'):
        ...
        sku_match = re.search(r'"sku"\s*:\s*"(\d{7,13})"', text)
        if sku_match:
...
def check_stock(html_text):
    """Check if product is in stock by looking for out-of-stock indicators."""
    ...
    for marker in out_of_stock_markers:
        if marker in text_lower:
            return False
    # Check for availability schema
    if '"instock"' in text_lower or '"in_stock"' in text_lower or 'availability.*instock' in text_lower:
        return True
    return True  # Default to in stock if no negative markers


def scrape(limit=0):
    """Run the Level 2 HTTP scraper for Atida."""
    engine = get_engine()
    ...
    urls = download_sitemap(SITEMAP_URL, filter_pattern=r'/es-es/')
    ...
    for i, url in enumerate(urls):
        try:
            ...
            # Custom EAN extraction if the generic one did not work
            if not data.get('ean'):
                data['ean'] = extract_ean_from_html(resp.text)
