"""
Scraper de Atida - Usa Playwright + Sitemap + Batch Fetch.

Atida (antes Mifarma) tiene un catalogo con paginas JS-heavy que requieren
un navegador real para resolver correctamente.

Estrategia:
  1. Abrir un navegador real con Playwright UNA SOLA VEZ
  2. Descargar el sitemap y extraer URLs de productos
  3. Desde el navegador, hacer fetch() en batches de 10 (paginas JS-heavy)
  4. Parsear precios del HTML con expresiones regulares
  5. Guardar en PostgreSQL y exportar a Excel

Uso (desde la raiz del proyecto via main.py):
    python main.py atida                  # Scraping completo
    python main.py atida --limit 1000     # Solo 1000 productos (test)
    python main.py atida --export         # Solo exportar a Excel
"""
...
SITEMAP_INDEX = "https://www.atida.com/es-es/sitemap.xml"
BATCH_SIZE_FETCH = 10   # Atida tiene paginas JS-heavy, lotes mas pequenos
BATCH_SIZE_DB = 200     # Productos por commit a la DB
DELAY_BETWEEN_BATCHES = 0.5  # Segundos entre batches

# JavaScript para extraer datos de producto desde HTML
# Atida usa meta tags product:price:amount y JSON-LD para datos de producto.
# Las paginas pueden tener precios originales en meta og o JSON-LD.
    ...
    return Promise.all(urls.map(url =>
        fetch(url)
            ...
            .then(html => {
                ...
                // 1. Nombre: h1 o og:title
                const h1Match = html.match(/<h1[^>]*>([^<]+)<\/h1>/i);
                if (h1Match) {
                    data.nombre = decodeEntities(h1Match[1].trim());
                } else {
                    const ogTitle = html.match(/og:title"\s*content="([^"]+)"/i);
                    if (ogTitle) data.nombre = decodeEntities(ogTitle[1].split('|')[0].trim());
                    else {
                ...
                // 2. Precio: meta product:price:amount o JSON-LD
                let precio = null;
                ...
                // 3. Precio original (PVP recomendado)
                const origPrice = html.match(/price--old[^>]*>([\d.,]+)/i) ||
                                  html.match(/was-price[^>]*>([\d.,]+)/i) ||
                                  html.match(/"highPrice"\s*:\s*"?([\d.]+)"?/i);
                if (origPrice) {
                    const orig = parseFloat(origPrice[1].replace(',', '.'));
                    if (precio && orig > precio) data.precioOriginal = orig;
                }

                // 4. EAN / SKU: JSON-LD sku, gtin13, or meta
                let sku = "";
                ...
                if (gtin) {
                    ...
                    if (skuMatch) sku = skuMatch[1];
                        ...
                        const eanMeta = html.match(/itemprop="sku"[^>]*content="(\d+)"/i);
                        if (eanMeta) sku = eanMeta[1];
                    }
                }
                data.sku = sku;

                // 5. Stock: JSON-LD availability o texto
                data.enStock = html.includes('InStock') ||
                               (!html.includes('Agotado') && !html.includes('Fuera de stock') &&
                                !html.includes('out-of-stock') && !html.includes('Sin stock'));
...
def descubrir_productos_sitemap(page):
    ...
    # Atida puede tener sub-sitemaps de productos o URLs directas
    product_subs = [u for u in sub_urls if 'product' in u.lower() or 'sitemap' in u.lower()]

    # Si hay sub-sitemaps, descargarlos
    if product_subs and any(u.endswith('.xml') for u in product_subs):
        logger.info(f"Encontrados {len(product_subs)} sub-sitemaps")
        productos_por_categoria = {}
        total = 0

        for i, sub_url in enumerate(product_subs):
            if not sub_url.endswith('.xml'):
                continue
            ...
            # Filtrar solo URLs de producto (con /es-es/ y no categorias)
        ...
        # Sitemap directo con URLs de producto
...
def ejecutar_scraping(db, limit=0):
    ...
    with sync_playwright() as p:
        ...
        logger.info("Abriendo pagina principal de Atida para iniciar contexto...")
        try:
            page.goto(f"{BASE_URL}/es-es/", wait_until="domcontentloaded", timeout=30000)
            time.sleep(3)
        except Exception as e:
            logger.error(f"Error cargando Atida: {e}")
            browser.close()
            return
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Scraper de precios Atida (Playwright + Sitemap)")
    ...
    try:
        if args.export:
            ...
            logger.info("=" * 60)
            logger.info("SCRAPING DE ATIDA (SITEMAP + BATCH FETCH)")
            logger.info("=" * 60)
