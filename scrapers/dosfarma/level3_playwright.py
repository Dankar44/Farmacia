"""
Scraper de DosFarma - Usa Playwright + Sitemap + Batch Fetch.

DosFarma tiene proteccion Cloudflare que bloquea peticiones HTTP directas.
Es necesario usar Playwright para resolver el challenge antes de hacer fetches.

Estrategia:
  1. Abrir un navegador real con Playwright UNA SOLA VEZ
  2. Navegar a la web para resolver el challenge de Cloudflare
  3. Descargar el sitemap y extraer URLs de productos
  4. Batch-fetch de paginas de producto con Promise.all() (10 a la vez)
  5. Parsear precios del HTML con expresiones regulares
  6. Guardar en PostgreSQL y exportar a Excel

Uso (desde la raiz del proyecto via main.py):
    python main.py dosfarma                  # Scraping completo
    python main.py dosfarma --limit 1000     # Solo 1000 productos (test)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, 'scraper_dosfarma.log'), encoding='utf-8'),
...
SITEMAP_INDEX = "https://www.dosfarma.com/sitemap.xml"
BATCH_SIZE_FETCH = 10   # DosFarma con Cloudflare, lotes pequenos
BATCH_SIZE_DB = 200     # Productos por commit a la DB
DELAY_BETWEEN_BATCHES = 0.8  # Mas pausa por Cloudflare
CF_WAIT_TIME = 8  # Segundos para resolver Cloudflare challenge

# JavaScript para extraer datos de producto desde HTML
# DosFarma usa PrestaShop con meta tags estandar y JSON-LD.
# Tiene Cloudflare pero una vez resuelto el challenge, fetch() funciona.
    ...
    return Promise.all(urls.map(url =>
        fetch(url)
            ...
            .then(html => {
                ...
                // 2. Precio: meta product:price:amount o itemprop price
                let precio = null;
                ...
                if (eanMatch) {
                    ...
                    if (gtin) {
                        ...
                        const skuMatch = html.match(/itemprop="sku"[^>]*content="(\d+)"/i) ||
                                         html.match(/itemprop="sku"[^>]*>(\d+)</i);
                        if (skuMatch) sku = skuMatch[1];
                        else {
                            const refMatch = html.match(/Referencia\s*:?\s*(\d{5,13})/i);
                ...
                // 5. Stock
                data.enStock = !html.includes('Agotado') &&
...
def descubrir_productos_sitemap(page):
    """Descarga el sitemap usando el contexto del navegador (necesario por Cloudflare)."""
    logger.info("Descargando sitemap index desde el navegador (Cloudflare)...")
    ...
    # DosFarma puede tener sub-sitemaps o URLs directas
    product_subs = [u for u in sub_urls if u.endswith('.xml') and ('product' in u.lower() or 'sitemap' in u.lower())]

    if product_subs:
        logger.info(f"Encontrados {len(product_subs)} sub-sitemaps")
        ...
        valid_urls = [u for u in sub_urls
                     ...
                     and u != BASE_URL
                     and u != BASE_URL + '/']
        logger.info(f"Total URLs descubiertas: {len(valid_urls)}")
...
def extraer_precios_batch(page, db, productos_por_categoria, limit=0):
    ...
    for batch_idx in range(total_batches):
        ...
        try:
            ...
            # Si hay error, puede ser que Cloudflare bloquee. Esperar mas.
            time.sleep(5)
            continue
...
def ejecutar_scraping(db, limit=0):
    ...
    with sync_playwright() as p:
        ...
        # DosFarma tiene Cloudflare: navegar y esperar a que se resuelva el challenge
        logger.info("Abriendo DosFarma para resolver Cloudflare challenge...")
        try:
            page.goto(BASE_URL, wait_until="domcontentloaded", timeout=30000)
            logger.info(f"Esperando {CF_WAIT_TIME}s para resolver Cloudflare...")
            time.sleep(CF_WAIT_TIME)
            # Verificar que se resolvio
            title = page.title()
            logger.info(f"Titulo de la pagina: {title}")
            if 'just a moment' in title.lower() or 'cloudflare' in title.lower():
                logger.warning("Cloudflare challenge no resuelto. Esperando mas tiempo...")
                time.sleep(10)
        except Exception as e:
            logger.error(f"Error cargando DosFarma: {e}")
            browser.close()
            return
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Scraper de precios DosFarma (Playwright + Sitemap)")
    ...
    try:
        if args.export:
            ...
            logger.info("=" * 60)
            logger.info("SCRAPING DE DOSFARMA (SITEMAP + BATCH FETCH)")
            logger.info("=" * 60)
