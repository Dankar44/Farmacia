"""
Scraper de FarmaciasDirect - Usa Playwright + Sitemap + Batch Fetch.

FarmaciasDirect tiene un catalogo accesible via sitemap con estructura
PrestaShop estandar y sin proteccion anti-bot estricta.

Estrategia:
  1. Abrir un navegador real con Playwright UNA SOLA VEZ
  2. Descargar el sitemap y extraer URLs de productos
  3. Batch-fetch de paginas de producto con Promise.all() (15 a la vez)
...
Uso (desde la raiz del proyecto via main.py):
    ...
    python main.py farmaciasdirect --limit 1000     # Solo 1000 productos (test)
...
SITEMAP_INDEX = "https://www.farmaciasdirect.com/sitemap.xml"
BATCH_SIZE_FETCH = 15   # Productos por batch de fetch (Promise.all)
...
# FarmaciasDirect usa una estructura tipica de ecommerce farmaceutico
# con meta tags, JSON-LD y microdata schema.org.
    ...
    return Promise.all(urls.map(url =>
        fetch(url)
            ...
            .then(html => {
                ...
                // 2. Precio: multiples fuentes
                let precio = null;
                // a) meta product:price:amount
                ...
                if (metaPrice) {
                    ...
                    if (itemPrice) {
                        ...
                        if (jsonPrice) precio = parseFloat(jsonPrice[1]);
                            ...
                            // d) Precio visible en span/div
                            const priceSpan = html.match(/class="[^"]*price[^"]*"[^>]*>([\d.,]+)\s*€/i);
                ...
                if (eanMatch) {
                    ...
                    if (gtin) {
                        ...
                        if (skuMatch) {
                            ...
                            // d) EAN en texto
                            const eanText = html.match(/EAN\s*:?\s*(\d{13})/i) ||
                                            html.match(/C(?:o|ó)digo\s*(?:de\s*)?barras\s*:?\s*(\d{13})/i);
                            if (eanText) sku = eanText[1];
                            else {
                                // e) Referencia
...
def descubrir_productos_sitemap(page):
    ...
    # Buscar sub-sitemaps
    product_subs = [u for u in sub_urls if u.endswith('.xml')]
    ...
    if product_subs:
        ...
        for i, sub_url in enumerate(product_subs):
            ...
            # Filtrar solo paginas de producto (no categorias, no imagenes)
...
def ejecutar_scraping(db, limit=0):
    ...
    with sync_playwright() as p:
        ...
        logger.info("Abriendo pagina principal de FarmaciasDirect para iniciar contexto...")
        try:
            page.goto(BASE_URL, wait_until="domcontentloaded", timeout=30000)
            time.sleep(2)
        except Exception as e:
            logger.error(f"Error cargando FarmaciasDirect: {e}")
            browser.close()
            return
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Scraper de precios FarmaciasDirect (Playwright + Sitemap)")
    ...
    try:
        if args.export:
            ...
            logger.info("=" * 60)
            logger.info("SCRAPING DE FARMACIASDIRECT (SITEMAP + BATCH FETCH)")
            logger.info("=" * 60)
