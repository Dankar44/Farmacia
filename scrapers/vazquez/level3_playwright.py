"""
Scraper de Farmacias Vazquez - Usa Playwright + Sitemap + Batch Fetch.

Farmacias Vazquez (farmavazquez.com) tiene un catalogo PrestaShop con
sitemap accesible y sin proteccion anti-bot estricta.

Estrategia:
  1. Abrir un navegador real con Playwright UNA SOLA VEZ
  2. Descargar el sitemap index y extraer URLs de productos
  3. Batch-fetch de paginas de producto con Promise.all() (15 a la vez)
  4. Parsear precios del HTML con expresiones regulares
  5. Extraer EAN desde la URL cuando es posible (patron numerico)
  6. Guardar en PostgreSQL y exportar a Excel

Uso (desde la raiz del proyecto via main.py):
    python main.py vazquez                  # Scraping completo
    python main.py vazquez --limit 1000     # Solo 1000 productos (test)
    python main.py vazquez --export         # Solo exportar a Excel
"""
...
SITEMAP_INDEX = "https://www.farmavazquez.com/gsitemap/2_index_sitemap.xml"
...
# FarmaVazquez usa PrestaShop con itemprop y meta tags estandar.
# Las URLs a menudo contienen codigos numericos que pueden ser EAN.
    ...
    return Promise.all(urls.map(url =>
        fetch(url)
            ...
            .then(html => {
                ...
                // 2. Precio: itemprop="price" content o meta product:price:amount
                let precio = null;
                ...
                // 4. EAN / SKU: buscar en multiples fuentes
                let sku = "";
                // a) ean13 en JSON de PrestaShop
...
def descubrir_productos_sitemap(page):
    ...
    product_subs = [u for u in sub_urls if u.endswith('.xml') and ('product' in u.lower() or 'gsitemap' in u.lower())]
...
def ejecutar_scraping(db, limit=0):
    ...
    with sync_playwright() as p:
        ...
        logger.info("Abriendo pagina principal de Farmacias Vazquez para iniciar contexto...")
        try:
            page.goto(BASE_URL, wait_until="domcontentloaded", timeout=30000)
            time.sleep(2)
        except Exception as e:
            logger.error(f"Error cargando Farmacias Vazquez: {e}")
            browser.close()
            return
...
def exportar_a_excel(db, filename=None):
    if filename is None:
        ...
        filename = os.path.join(export_dir, 'precios_vazquez.xlsx')
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Precios FarmaVazquez"

    # Estilo azul Vazquez
    header_font = Font(name='Calibri', bold=True, color='FFFFFF', size=11)
    header_fill = PatternFill(start_color='1565C0', end_color='1565C0', fill_type='solid')
    header_alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    fill_par = PatternFill(start_color='E3F2FD', end_color='E3F2FD', fill_type='solid')
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Scraper de precios Farmacias Vazquez (Playwright + Sitemap)")
    ...
    try:
        if args.export:
            ...
            logger.info("=" * 60)
            logger.info("SCRAPING DE FARMACIAS VAZQUEZ (SITEMAP + BATCH FETCH)")
            logger.info("=" * 60)
