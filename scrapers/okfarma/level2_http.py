"""
Level 2 HTTP scraper for OkFarma (okfarma.es)
...
SITEMAP_URL = "https://okfarma.es/sitemap-1.xml"
BATCH_SIZE = 100
...
def extract_price_custom(html_text):
    """Extract price with OkFarma-specific logic."""
    ...
    # Try price_pvp in script tags (common in custom JS)
    for script in soup.find_all('script'):
        text = script.get_text()
        match = re.search(r'price_pvp["\']?\s*[:=]\s*["\']?([\d.,]+)', text)
        if match:
            price = extract_price(match.group(1))
            if price:
                return price
        # Also try "price" key in JS objects
...
def extract_ean_custom(html_text):
    """Extract EAN from OkFarma page, trying JSON data and itemprop."""
    from bs4 import BeautifulSoup
    soup = BeautifulSoup(html_text, 'lxml')

    # Try ean13 in script tags (JSON or JS variable)
    for script in soup.find_all('script'):
        text = script.get_text()
        # Look for ean13 in JSON
...
def check_stock(html_text):
    """Check stock by looking for Agotado or availability markers."""
    ...
    # Check JSON-LD or schema availability
    if '"outofstock"' in text_lower:
        return False
    if '"instock"' in text_lower:
        return True

    # Check for quantity/availability in JS
    if re.search(r'quantity["\']?\s*[:=]\s*0\b', text_lower):
        return False
...
def scrape(limit=0):
    """Run the Level 2 HTTP scraper for OkFarma."""
    engine = get_engine()
